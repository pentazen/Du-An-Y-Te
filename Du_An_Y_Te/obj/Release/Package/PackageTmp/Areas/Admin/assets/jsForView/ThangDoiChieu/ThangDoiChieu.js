...
function DeleteTaiKhoan(taikhoan, id, them) {
    ...
    swal({
        title: "Are you sure?",
        text: "This Parameter Interval ( " + taikhoan + " ) Will Delete",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        confirmButtonText: "Yes",
     ...
     function (isConfirm) {
         if (isConfirm) {
             var url = document.location.origin + "/Admin/ThangDoiChieu/XoaThangDoiChieu";
             $.ajax({
                 method: "POST",
                 url: url,
           ...
           .done(function (data) {
               ...
               if (data == true || data == "True") {
                   swal("Deleted!", "Parameter Interval " + taikhoan + " has been remove!", "success");
                   table.row('.selected').remove().draw(true);
               }
               else {
                   return false;
